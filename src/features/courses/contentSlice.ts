...
export const addCourse = createAsyncThunk('courses/add', async (courseData: NewCourseData, { rejectWithValue }) => {
    try {
...
export const updateCourse = createAsyncThunk('courses/update', async ({ courseId, courseData }: { courseId: string, courseData: UpdateCourseData }, { rejectWithValue }) => {
    try {
...
export const deleteCourse = createAsyncThunk('courses/delete', async (courseId: string, { rejectWithValue }) => {
    try {
...
export const fetchContentForForm = createAsyncThunk('content/fetchForForm', async (_, { rejectWithValue }) => {
    try {
        const [videosRes, audiosRes, ebooksRes] = await Promise.all([
        ...
        return {
            ...
            audios: audiosRes.data.audios || [],
            ebooks: ebooksRes.data.ebooks || []
        };
    } catch (error: any) {
...
const courseSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchContentForForm.fulfilled, (state, action) => {
