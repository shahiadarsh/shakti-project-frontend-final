    ...
    return (
        <div className="w-full">
            <div className="aspect-video bg-black rounded-lg overflow-hidden mb-6">
                {currentVideo ? (
                    <video key={currentVideo._id} src={currentVideo.videoFileUrl} controls autoPlay controlsList="nodownload" className="w-full h-full">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                ...
                {Array.from({ length: 3 - totalUnlocked }).map((_, index) => (
                    <div key={index} className="w-full flex items-center gap-4 p-3 rounded-lg text-left bg-dark-main cursor-not-allowed">
                        ...
                        <span className="font-medium text-text-secondary/50">Day {totalUnlocked + index + 1}: Not Unlock Yet</span>
                    </div>
                ))}
            </div>
        </div>
